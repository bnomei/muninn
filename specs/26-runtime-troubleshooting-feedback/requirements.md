...
## EARS requirements
1. When the binary receives `__debug_record [seconds]`, the system shall load config, refresh recording permissions, capture a one-shot WAV using the configured recording settings, print basic recording metadata, and exit without starting the tray runtime.
