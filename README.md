...
When `mcp_enabled = true`, Muninn runs a streamable-HTTP [MCP](https://modelcontextprotocol.io) server at `http://<mcp_bind_address>/mcp` (default `http://127.0.0.1:2769/mcp`) exposing four tools: `get_status`, `start_recording`, `stop_recording`, and `cancel_recording`. (The `muninn://toggle` URL verb has no MCP equivalent — agents should use the explicit start/stop tools.)
